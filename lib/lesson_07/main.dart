...
void greetPositionalOptional([String? name, String? timeOfDay]) {
  String finalName = name ?? 'друже';
  String finalTime = timeOfDay ?? 'дня';
...
void sendMessage(String message, String recipient, [bool isUrgent = false]) {
  String urgency = isUrgent ? '[ТЕРМІНОВО] ' : '';
  print('${urgency}Повідомлення для $recipient: $message');
}

void bookTicket(String from, String to,
    {bool priority = false, String? seatClass}) {
  String ticket = 'Квиток: $from → $to';
...
void returnValuesExample() {
  ...
  String fullName = createFullName('Іван', 'Петренко');
  print('Повне ім\'я: $fullName');

  bool isAdult = checkAge(20);
  print('Повнолітній: $isAdult');

  // Функція може повертати null
  String? findUserResult = findUser('admin');
...
void scopeExample() {
  ...
  String localMessage = 'Це локальна змінна';
  print(localMessage);
  ...
  // Змінні в блоках {}
  if (true) {
    String blockVariable = 'Змінна в блоці if';
    print(blockVariable);
  }
  ...
  for (int i = 0; i < 3; i++) {
    String loopVariable = 'Ітерація $i';
    print(loopVariable);
  }
...
void demonstrateScope(String parameter) {
  ...
  String localVar = 'Локальна змінна функції';
